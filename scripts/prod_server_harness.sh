...
wait_for_server_start() {
  ...
  while true; do
    if ! kill -0 "${SERVER_PID}" 2>/dev/null; then
      echo "Production server exited before startup completed." >&2
    ...
    if (( "$(date +%s)" - start_ts >= timeout_seconds )); then
      echo "Timed out waiting for production server startup." >&2
      tail -n 200 "${LOG_FILE}" >&2 || true
      return 1
    fi
...
main() {
  ...
  wait_for_server_start "${1:-180}"
  echo "Production Forge server started successfully with staged mods."
}
