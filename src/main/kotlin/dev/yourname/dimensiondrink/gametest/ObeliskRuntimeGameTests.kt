...
class ObeliskRuntimeGameTests {
    @GameTest(templateNamespace = "dimensiondrink", template = "bootstrap/empty", batch = "obelisk_runtime", timeoutTicks = 220)
    fun runtime_service_lists_and_finds_loaded_dimensiondrink(helper: GameTestHelper) {
        ObeliskGameTestSupport.runtimeServiceListsAndFindsLoadedDimensionDrink(helper)
    }

    @GameTest(templateNamespace = "dimensiondrink", template = "bootstrap/empty", batch = "obelisk_runtime", timeoutTicks = 120)
    fun terrain_clearing_task_is_removed(helper: GameTestHelper) {
        ObeliskGameTestSupport.terrainClearingTaskIsRemoved(helper)
    }

    @GameTest(templateNamespace = "dimensiondrink", template = "bootstrap/empty", batch = "obelisk_runtime", timeoutTicks = 200)
    ...
    @GameTest(templateNamespace = "dimensiondrink", template = "bootstrap/empty", batch = "obelisk_runtime", timeoutTicks = 260)
