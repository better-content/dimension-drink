...
import dev.yourname.dimensiondrink.api.ObeliskApi
...
import dev.yourname.dimensiondrink.data.CultivationPaletteDefinition
...
import dev.yourname.dimensiondrink.runtime.ObeliskRuntimeService
...
import dev.yourname.dimensiondrink.runtime.run.RunState
...
object ObeliskGameTestSupport {
            ...
            waitForPreparedTemplate(helper, "end") {
                ...
                waitUntil(helper, 220, "Expected charged obelisk activation to create a run record", condition = {
                    ...
                    waitUntil(helper, 600, "Expected charged obelisk activation to produce an active canonical run", condition = {
                        ...
                        val activeRunId = (helper.level.getBlockEntity(obeliskPos) as? ObeliskBlockEntity)?.activeRunId
                        val run = activeRunId?.let(RunRegistry::get)
                        run?.state == dev.yourname.dimensiondrink.runtime.run.RunState.ACTIVE &&
    ...
    fun runtimeServiceListsAndFindsLoadedDimensionDrink(helper: GameTestHelper) {
        ...
        helper.assertTrue(
            ...
            "Expected runtime service listLoaded to include both placed dimensiondrink"
        )
    ...
    private fun beginRunOrFail(server: net.minecraft.server.MinecraftServer, definitionId: String): dev.yourname.dimensiondrink.api.RunHandle {
