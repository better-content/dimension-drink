...
class DimensionalFontWorldgenPlacementTest {
    ...
    fun structureSetOwnsNaturalDimensionalFontPlacement() {
        val structureSet = assertNotNull(
            javaClass.classLoader.getResource("data/dimension_drink/worldgen/structure_set/dimensional_fonts.json")
        ).readText()
        val structure = assertNotNull(
            javaClass.classLoader.getResource("data/dimension_drink/worldgen/structure/dimensional_font.json")
        ).readText()
        val biomeTag = assertNotNull(
            javaClass.classLoader.getResource("data/dimension_drink/tags/worldgen/biome/has_structure/dimensional_font.json")
        ).readText()
        ...
        assertTrue(
            Regex(""""type"\s*:\s*"dimension_drink:dimensional_font"""").containsMatchIn(structure),
            "Natural dimensional font placement should use the custom structure type"
        )
        assertTrue(
            Regex(""""biomes"\s*:\s*"#dimension_drink:has_structure/dimensional_font"""").containsMatchIn(structure),
        ...
        assertFalse(javaClass.classLoader.getResource("data/dimension_drink/worldgen/placed_feature/dimensional_font_placed.json") != null)
        assertFalse(javaClass.classLoader.getResource("data/dimension_drink/worldgen/configured_feature/dimensional_font.json") != null)
        assertFalse(javaClass.classLoader.getResource("data/dimension_drink/forge/biome_modifier/add_dimensional_font.json") != null)
    }
